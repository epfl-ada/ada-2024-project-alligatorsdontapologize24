...
def plot_coefficients_with_confidence_intervals(results_df: pd.DataFrame):
    """
    Plot the coefficients and 95% confidence intervals for all variables, excluding 'const'.
    
    Args:
        results_df (pd.DataFrame): A DataFrame with coefficient, standard error, 
                                    and confidence intervals ('lower_ci' and 'upper_ci') columns.
                                    Excludes 'const' from plotting.
    """
