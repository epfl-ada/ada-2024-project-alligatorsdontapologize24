...
class GPT4mini_ViolenceClassifier_extreme:
    def __init__(self):
        #API parameters
        #key (toc toc)
        ...
        self.Instruction = "Classify each plot between violent and non-violent. Respond with a dictionary where the keys are the plot numbers (e.g., 'plot1', 'plot2') and the values are the levels of violence ('Non-violent', 'Violent')"
        
        self.Example = """Here are some examples for each label :
        - **Non-violent**: plot1 :'norma and malcolm miochaels are a middle-aged married couple who are in the midst of a midlife crisis. both decide to separate and begin their lives anew away from each other. however, problems ensue once they discover that they are no longer as young as they used to be.'
        ...
        self.function = {
           ...
           "description": "Binary classification of the level of violence of a list of movie plots",
           "parameters": {
               "type": "object",
               "properties": {
                   "prediction": {
